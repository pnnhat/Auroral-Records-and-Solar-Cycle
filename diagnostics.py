...
Multimodality diagnostics for NHPP period inference on historical aurora records.

All functions here are read-only with respect to the existing inference pipeline.
They add *diagnostics*; they do not filter, delete, or override posterior samples.

Functions
---------
profile_logL_over_P    Profile log-likelihood of P, maximised over (beta0, a, b).
                       The NHPP log-likelihood is strictly concave in these three
                       parameters for any fixed omega, so a single L-BFGS-B run
                       with analytical gradients reliably finds the global optimum.

windowed_period_scan   Slide a fixed-width window over the record and compute the
                       profile logL inside each window.  Reveals nonstationarity
                       and shows whether the dominant period is stable across epochs.

run_sbc                Simulation-based calibration.  Draw (P_true, phi_true,
                       beta1_true) from the prior, simulate event times, run the
                       profile estimator, and record whether P_true is recovered.
                       A well-calibrated estimator has a uniform rank histogram.

plot_windowed_scan     Two-panel figure for windowed scan results.
plot_sbc               Two-panel figure for SBC results.
plot_ppc_epoch         Posterior predictive checks split by time epoch, using
                       flat samples from the (beta0, a, b, logP) MCMC.
"""
...
# ---------------------------------------------------------------------------
# 1.  Profile log-likelihood over P
# ---------------------------------------------------------------------------
...
def plot_windowed_scan(results, figsize=(14, 5)):
    ...
    plt.colorbar(
        plt.cm.ScalarMappable(norm=norm, cmap=cmap), ax=ax_l,
    ...
    sc = ax_r.scatter(
        midpoints, phats, c=midpoints, cmap="plasma", s=sizes,
        vmin=midpoints.min(), vmax=midpoints.max(), zorder=3
    )
...
# ---------------------------------------------------------------------------
# 5.  Posterior predictive checks split by epoch
# ---------------------------------------------------------------------------
...
def plot_ppc_epoch(
    ...
    for i, ax in enumerate(axes):
        ...
        ax.step(centers, counts / bw, where="mid", color="black", lw=2,
...
# ---------------------------------------------------------------------------
# 6.  Compare 1-harmonic vs 2-harmonic profile logL
# ---------------------------------------------------------------------------

def _neg_ll_and_grad_2harm(params, sin_ev, cos_ev, sin2_ev, cos2_ev, N, T, omega, ll_grid):
...
def plot_profile_comparison(
    event_times, T, P_grid,
    ll_grid=4000, figsize=(10, 4)
):
    """
    ...
    ax.plot(P_grid, prof2 - prof2.max(), lw=2, label="2-harmonic", color="darkorange",
            ls="--")
