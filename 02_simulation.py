## Simulation study: NHPP feasibility
# Can N ~ 787 events over T ~ 474 yr recover an 11-yr cycle? Simulate from a known NHPP and try to recover it.
# All model/likelihood/sampler functions are imported from helpers.py.
import numpy as np
...
## Ground-truth parameters for the synthetic catalogue
# T observation span, P_true target period, beta1 modulation depth, N_target expected event count
...
anim.save("plot/animation.gif", writer=PillowWriter(fps=10))
plt.show()
...
def update_seq(frame_idx):
    ...
    ax_P_seq.axvline(
        P_hat, linestyle=":", color="blue", label=f"Estimated period = {P_hat:.2f}"
    )
...
anim_seq.save("plots/animation_sequential.gif", writer=PillowWriter(fps=10))

plt.show()

## What's the minimum number of events needed to get a good estimate of the period?
## Minimum events for stable recovery (chronological)
# Add events one at a time; record when P_hat stays within +/-0.5 yr of P_true
...
## Survival curve (empirical CDF of minimum events)
...
## Random-order recovery animation across the full span
seed = 2200
rng = np.random.default_rng(seed)
...
anim.save("plots/random_animation.gif", writer=PillowWriter(fps=15))
plt.show()

## Random-order recovery animation
seed = 2200
rng = np.random.default_rng(seed)
...
anim_rand.save("plots/animation_random.gif", writer=PillowWriter(fps=10))

plt.show()

## Minimum events for stable recovery (random order)
seed = 2200
rng = np.random.default_rng(seed)
...
## Success curve (empirical CDF of minimum events)
N_max = int(np.nanmax(jv))
Ns = np.arange(1, N_max + 1)
...
## Minimum-events stability across multiple seeds
...
## Two-harmonic intensity model


## One-harmonic intensity model
## true 2-harmonic, fit 1-harmonic
...
## Minimum events for 1- vs 2-harmonic models
...
## MCMC on the simulated catalogue
